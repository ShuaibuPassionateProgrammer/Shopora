...
const addProduct = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        switch (true) {
            ...
            case !brand:
                return res.json({ error: "Brand is required" }) as any;
            case !description:
                return res.json({ error: "Description is required" }) as any;
            case !price:
                return res.json({ error: "Price is required" }) as any;
            case !category:
                return res.json({ error: "Category is required" }) as any;
            case !quantity:
                return res.json({ error: "Quantity is required" }) as any;
        }
...
const updateProductDetails = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
        const { name, description, price, category, quantity, brand } = req.fields;
...
const fetchProductById = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (product) {
            return res.json(product) as any;
        } else {
            res.status(404);
...
const addProductReview = asyncHandler(async (req: Request, res: Response): Promise<void> => {
    try {
